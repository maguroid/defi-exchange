...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint256 _amount) public payable returns (uint256) {
        ...
        if (cryptoDevTokenReserve == 0) {
            ...
            /* liquidity ratio's fomula
            cryptoDevTokenAmount / cryptoDevTokenReserve = eth sent(msg.value) / ethReserve
            */
            uint256 cryptoDevTokenAmount = (msg.value * cryptoDevTokenReserve) /
                ethReserve;
            require(
                _amount >= cryptoDevTokenAmount,
                "Amount of tokens sent is less than minimum tokens required"
            );
